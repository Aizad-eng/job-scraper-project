...
**ScrapingDog limits.** The account allows 5 concurrent requests by default; set `SCRAPINGDOG_CONCURRENCY` in the environment if your plan allows more. One shared limiter covers the AI Mode check and the Google domain lookup across every running job, so it never goes above 5 in flight; 429 / 5xx / network errors are retried up to 3 times with 2 s, 4 s waits, while a 402 (no credits) fails straight away and falls through to the fallbacks.

**When Claude has no credit.** The Anthropic API answers "Your credit balance is too low". The app then pauses all Claude calls for an hour and, because most AI Mode answers are prose Claude has to structure, also stops sending companies to ScrapingDog for that hour so credits are not spent on answers nothing can read. Companies fall through to the optional GPT / Perplexity fallbacks or are kept unchecked. Salary extraction and title cleanup from descriptions pause too. Log lines are prefixed with the service that failed (`Claude (Anthropic API): …` or `ScrapingDog failed for …`).
